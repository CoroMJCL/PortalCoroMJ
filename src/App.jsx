...
function ComunidadesWidget({ comunidades, isAdmin, setSection }) {
    ...
    return (
      <Card style={{ marginTop: 14 }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:8 }}>
          <div style={{ fontFamily:"'Poppins',sans-serif", fontSize:13, fontWeight:700, color:C.dark }}>⛪ Comunidades</div>
          <button onClick={() => setSection("admin")} style={{ background:C.primaryLight, border:`1px solid ${C.primary}50`, borderRadius:7, padding:"4px 10px", fontSize:11, fontWeight:600, color:C.primary, cursor:"pointer" }}>+ Agregar</button>
        </div>
        <div style={{ borderRadius:10, background:"#f0f9ff", border:"2px dashed #0ea5e940", display:"flex", alignItems:"center", justifyContent:"center", padding:"24px", fontSize:12, color:C.gray, textAlign:"center" }}>
          Agrega comunidades desde <strong>Admin → Comunidades</strong>
        </div>
      </Card>
    );
  }
  ...
  return (
    <Card style={{ marginTop: 14, padding: 0, overflow: "hidden" }}>
      {/* Foto compacta */}
      <div key={idx} style={{ position:"relative", width:"100%", aspectRatio:"4/3", background:"#1e293b", overflow:"hidden" }}>
        {com.foto_url
        ...
        {comunidades.length > 1 && (
          <>
            <button onClick={prev} disabled={idx===0} style={{ position:"absolute", left:8, top:"50%", transform:"translateY(-50%)", width:32, height:32, borderRadius:8, border:"none", background:"rgba(0,0,0,0.45)", color:"white", fontSize:18, cursor:idx===0?"default":"pointer", display:"flex", alignItems:"center", justifyContent:"center", opacity:idx===0?0.3:0.9, transition:"all 0.15s" }}>‹</button>
            <button onClick={next} disabled={idx===comunidades.length-1} style={{ position:"absolute", right:8, top:"50%", transform:"translateY(-50%)", width:32, height:32, borderRadius:8, border:"none", background:"rgba(0,0,0,0.45)", color:"white", fontSize:18, cursor:idx===comunidades.length-1?"default":"pointer", display:"flex", alignItems:"center", justifyContent:"center", opacity:idx===comunidades.length-1?0.3:0.9, transition:"all 0.15s" }}>›</button>
          </>
        )}
        {/* Puntos indicadores */}
        {comunidades.length > 1 && (
          <div style={{ position:"absolute", bottom:8, left:0, right:0, display:"flex", justifyContent:"center", gap:5 }}>
            {comunidades.map((_,i) => (
              <div key={i} onClick={() => setIdx(i)} style={{ width:i===idx?16:6, height:6, borderRadius:3, background:i===idx?"white":"rgba(255,255,255,0.5)", cursor:"pointer", transition:"all 0.2s" }} />
            ))}
          </div>
        )}
        {/* Badge admin */}
        {isAdmin && (
          <button onClick={() => setSection("admin")} style={{ position:"absolute", top:8, right:8, background:"rgba(0,0,0,0.45)", border:"none", color:"white", fontSize:10, fontWeight:600, borderRadius:6, padding:"3px 8px", cursor:"pointer" }}>✏️ Gestionar</button>
        )}
      </div>

      {/* Info debajo */}
      <div style={{ padding:"12px 14px" }}>
        <div style={{ fontFamily:"'Poppins',sans-serif", fontSize:14, fontWeight:700, color:C.dark, marginBottom:6 }}>{com.nombre}</div>
        {com.direccion && (
          <div style={{ display:"flex", alignItems:"flex-start", gap:6, marginBottom:4 }}>
            <span style={{ fontSize:12, flexShrink:0, marginTop:1 }}>📍</span>
            <span style={{ fontSize:12, color:C.gray, lineHeight:1.4 }}>{com.direccion}</span>
          </div>
        )}
        {com.telefono && (
          <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:4 }}>
            <span style={{ fontSize:12 }}>📞</span>
            <a href={`tel:${com.telefono}`} style={{ fontSize:12, color:C.dark, textDecoration:"none" }}>{com.telefono}</a>
          </div>
        )}
        {com.email && (
          <div style={{ display:"flex", alignItems:"center", gap:6 }}>
            <span style={{ fontSize:12 }}>✉️</span>
            <a href={`mailto:${com.email}`} style={{ fontSize:11, color:"#0ea5e9", textDecoration:"none", wordBreak:"break-all" }}>{com.email}</a>
          </div>
        )}
        {com.descripcion && (
          <div style={{ fontSize:11, color:C.gray, marginTop:8, lineHeight:1.5, borderTop:`1px solid ${C.border}`, paddingTop:8 }}>{com.descripcion}</div>
        )}
      </div>
    </Card>
  );
